...
\begin{examplebox}[\textbf{Example: }Checking polynomial identities using a randomized algorithm]
    ...
    This event occurs if and only if $\displaystyle r$ is a root of the polynomial $\displaystyle H(x) = F(x) - G(x)$: since $\displaystyle H(x)$ will always have degree at most $\displaystyle d$, it will have at most $\displaystyle d$ roots, allowing to compute the error probability as follows:
